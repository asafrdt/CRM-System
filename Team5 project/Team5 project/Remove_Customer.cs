...
namespace Team5_project
{
    public partial class Remove_Customer : Form
    {
        ...
        private void SubmitBox_Click(object sender, EventArgs e)
        {
            ...
            if (dt2.Rows.Count == 0)
            {
                MessageBox.Show("the serial number not exist in the system, please try again", "alert", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                conn.Open();
                ...
                cmd.ExecuteNonQuery();
                conn.Close();
                MessageBox.Show("Customer has been deleted", "alert", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}
